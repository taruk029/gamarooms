...
@push('styles')
<link href="{{ asset('backend/datatables/assets/vendors/custom/datatables/datatables.bundle.css') }}" rel="stylesheet" type="text/css" />
<style type="text/css">
    .kt-portlet--mobile
{
        ...
        <table class="table table-striped- table-bordered table-hover table-checkable" id="kt_table_1">
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td>
                            @if(!empty($row->image_name))
                                @if(file_exists(base_path().'/img/side_bannners/'.$row->image_name))   
                                    <img src="{{$row->image_url}}" class="img-thumb img" > 
                                @else
                                     <img src="{{$row->image_url}}" class="img-thumb img"> 
                                @endif
                            @endif
                    ...
                    <td>
                        <a class="btn btn-sm btn-clean btn-icon btn-icon-md" href="javascript:void(0)"  onclick="javascript:open_modal({{$row->id}})" title="Change Image" >
  ...
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      <div class="modal-header">
        ...
        <form action="{{url('admin/change_image')}}" method="post" enctype='multipart/form-data'>
        @csrf
...
@endsection
@push('scripts')
<script src="{{ asset('backend/datatables/assets/vendors/custom/datatables/datatables.bundle.js') }}" type="text/javascript"></script>
<script src="{{ asset('backend/datatables/assets/js/demo8/pages/crud/datatables/basic/paginations.js') }}" type="text/javascript"></script>
<script src="{{ asset('backend/assets/js/demo8/pages/bootstrap-datepicker.js') }}" type="text/javascript"></script>
<!--end::Page Scripts -->
<script>
    function form_reset()
    {
